...
defineSupportCode(({Given, When, Then, Before}) => {
    ...
    When('the builder is initialised with the id {stringInQuotes}', function (id, callback) {
        try {
    ...
    Then('the result user should have an id {stringInDoubleQuotes}', function (userId, callback) {
        expect(this.user.getId()).to.equal(userId);
        callback();
    });

    Then('the result user should have a attribute with key {stringInDoubleQuotes} and value {stringInDoubleQuotes}', function (key, value, callback) {
        expect(this.user.getAttributesForKey(key)).to.include(value);
        expect(this.user.getAttributesForKey(key)).to.have.lengthOf(1);
        callback();
    });

    Then('the result user should have a attribute with key {stringInDoubleQuotes} and current datetime in iso8601', function (key, callback) {
